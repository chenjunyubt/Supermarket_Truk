...
import com.supermarket.utils.StorageUtils;
...
/*		else {
	...
	private boolean handleException(Throwable ex) {
		...
		//处理信息   
//		gotoErrorReportDialogActivity(ex);
		return true;
	}

	/*private void gotoErrorReportDialogActivity(Throwable ex) {
		final Intent intent = new Intent(mContext, ErrorReportActivity.class);
		intent.putExtra(ErrorReportActivity.KEY_PID, android.os.Process.myPid());
		intent.putExtra(ErrorReportActivity.KEY_THROWABLE, ex);
	...
	private void dumpBuildDetails(OutputStream os) throws IOException {
		...
		// Rom
		os.write("\nTotal=".getBytes());
		os.write(Long.toString(totalBlocks * blockSize / 1048576).getBytes());
		os.write("\nAvailable=".getBytes());
		os.write(Long.toString(availableBlocks * blockSize / 1048576).getBytes());
		os.write('\n');
	}
