...
public class NetUtils {	
	...
	public static int getNetConnectType(Context context) {
        ...
        if (((wifi != null) && wifi.isAvailable() && wifi.isConnectedOrConnecting())
        	...
        	TTLog.d("getNetConnectType:----- ConnectType = ------"+"NETWORK_TYPE_ALL");
        } else if ((wifi != null) && wifi.isAvailable() && wifi.isConnectedOrConnecting()) {
        	res = NETWORK_TYPE_WIFI;
        	TTLog.d("getNetConnectType:----- ConnectType = ------"+"NETWORK_TYPE_WIFI");
        } else if ((mobile != null) && mobile.isAvailable() && mobile.isConnectedOrConnecting()) {
        	res = NETWORK_TYPE_MOBILE;
        	TTLog.d("getNetConnectType:----- ConnectType = ------"+"NETWORK_TYPE_MOBILE");
        } else {
        	res = NETWORK_TYPE_UNKNOWN;
        	TTLog.d("getNetConnectType:----- ConnectType = ------"+"NETWORK_TYPE_UNKNOWN");
        }        
        TTLog.d("getNetConnectType:----- end ------");
        return res;       
    }
...
//				case TelephonyManager.NETWORK_TYPE_HSUPA:// ~ 1-23 Mbps
	...
	public static boolean isNetConnected() {
		return getNetConnectType(TTApplication.getAppContext())!=NETWORK_TYPE_UNKNOWN;
	}
}
